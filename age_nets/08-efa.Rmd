...
efa_fun <- function(inventory, set, age, rule){
  ...
  pdf(file = sprintf("~/Box/networks/SAPA/02-results/01-plots/08-efa/%s-%s-%s-%s.pdf"
  ...
  save(res, file = sprintf("~/Box/networks/SAPA/02-results/efa-%s/%s-%s-%s.RData"
...
loadRData <- function(file, set, obj){
    path <- sprintf("~/Box/networks/SAPA/02-results/efa-%s/%s",
                               set, file)
    #loads an RData file, and returns it
    load(path)
    get(ls()[grepl(obj, ls())])
...
efa_nested <- tibble(
  file = list.files("~/Box/networks/SAPA/02-results/efa-age")
  , set = "age"
  ) %>% full_join(tibble(
    file = list.files("~/Box/networks/SAPA/02-results/efa-decades")
    , set = "decades"
  )) %>%
...
save_fun <- function(graph, age){
  pdf(file = sprintf("~/Box/networks/SAPA/results/plots/efa/efa_%s.pdf", age), 
      width = 6, height = 6)
  plot(graph)
  title(age)
  dev.off()
