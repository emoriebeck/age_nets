...
traj_plot_fun <- function(mt, inventory, rule, gamma, set){
  ...
  ggsave(p, file = sprintf("~/Box/networks/SAPA/02-results/01-plots/02-trajectories/png/%s-%s-%s-%s.png"
                        , inventory, rule, str_remove_all(gamma, "[.]"), set)
         , width = 5, height = 4)
  ggsave(p, file = sprintf("~/Box/networks/SAPA/02-results/01-plots/02-trajectories/pdf/%s-%s-%s-%s.pdf"
                        , inventory, rule, str_remove_all(gamma, "[.]"), set)
         , width = 5, height = 4)
  return(p)
}
...
edges.sum %>%
  ...
  ggplot(aes(x = as.numeric(age), y = mean)) + 
      ...
      theme(axis.text.x = element_text(size = rel(.75), face = "bold", angle = 90),
            ...
            legend.title = element_text(size = rel(.7), face = "bold"),
            legend.position = c(.15, .9))
ggsave("~/Box/networks/SAPA/02-results/01-plots/02-trajectories/differentiation_coherence_line_graph_comp.png",
       width = 15, height = 8)
ggsave("~/Box/networks/SAPA/02-results/01-plots/02-trajectories/differentiation_coherence_line_graph_comp.pdf",
       width = 15, height = 8)
```
...
st_traj_fun <- function(mt, st, inventory, rule, gamma, set){
  ...
  ggsave(file = sprintf("~/Box/networks/SAPA/02-results/01-plots/03-st-trajectories/png/%s-%s-%s-%s.png"
                        , inventory, rule, str_remove_all(gamma, "[.]"), set)
         , width = 6, height = 4)
  ggsave(file = sprintf("~/Box/networks/SAPA/02-results/01-plots/03-st-trajectories/pdf/%s-%s-%s-%s.pdf"
                        , inventory, rule, str_remove_all(gamma, "[.]"), set)
         , width = 6, height = 4)
  return(p)
}
