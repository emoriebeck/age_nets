...
nmi_plot_fun <- function(df, inventory, rule, set, gamma){
  ...
  ggsave(p, file = sprintf("~/Box/networks/SAPA/02-results/01-plots/07-nmi/png/%s-%s-%s-%s.png"
                        , inventory, rule, set, gamma)
         , width = 8, height = 8)
  ggsave(p, file = sprintf("~/Box/networks/SAPA/02-results/01-plots/07-nmi/pdf/%s-%s-%s-%s.pdf"
                        , inventory, rule, set, gamma)
         , width = 8, height = 8)
  return(p)
}
...
match_matall     <- communitiesall     %>% mutate(match = map2(data, inventory, possibly(match_fun, NA_real_)))
...
merge_comm_fun <- function(all, u38, p39, inventory, rule, gamma){
  ...
  ggsave(p2, file = sprintf("~/Box/networks/SAPA/02-results/01-plots/05-community-matching/pdf/%s-%s-%s.pdf"
                        , inventory, rule, gamma)
         , width = 9, height = 6)
  ggsave(p2, file = sprintf("~/Box/networks/SAPA/02-results/01-plots/05-community-matching/png/%s-%s-%s.png"
                        , inventory, rule, gamma)
         , width = 9, height = 6)
  return(p2)
}
